...
council_pop$weight[
  council_pop$country == "Croatia" & council_pop$ysp < 2014
] <- NA
council_pop$weight[
  council_pop$country == "United Kingdom" & council_pop$ysp >= 2018
] <- NA
...
council_pop <- council_pop %>%
  ...
  dplyr::group_by(ysp) %>%
  dplyr::mutate(
    pct_weight = weight / sum(weight),
    pct_pop = pop_int / sum(pop_int)
  )
...
dfg <- subset(
  df_merged_FULL,
  select = which(!duplicated(names(df_merged_FULL)))
)
