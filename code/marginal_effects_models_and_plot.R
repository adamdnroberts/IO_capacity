...
etable(models_noA$rev, models_noA$exp, tex = FALSE, digits = 3)
etable(models_noEOY$rev, models_noEOY$exp, tex = FALSE, digits = 3)

#plot
# Extract coefficients and confidence intervals
extract_coefs <- function(model) {
  coef <- coef(model)["ecfin:diff_iv"]
  ci90 <- confint(model, level = 0.9)["ecfin:diff_iv", ]
  ci95 <- confint(model, level = 0.95)["ecfin:diff_iv", ]
  list(
    coef = coef,
    ci_lower90 = ci90[1],
    ci_upper90 = ci90[2],
...
print(guide_diff_results)

ggsave(
  filename = "C:/Users/adamd/Dropbox/Apps/Overleaf/EU_Capacity/images/guide_diff_results_plot.pdf",
  plot = guide_diff_results,
  width = 6,
  height = 4
)
