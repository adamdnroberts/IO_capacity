...
nat11 <- read.csv(paste0(rawpath,"2011-Europa_SP2_BS_Nat_x_DG_201112.csv"), header=FALSE)
nat11 <- rbind(nat11, c("date", rep("2011-12-01",length(nat11)-1)))

nat12 <- read.csv(paste0(rawpath,"2012-Europa_SP2_BS_Nat_x_DG_201212.csv"), header=FALSE)
nat12 <- rbind(nat12, c("date", rep("2012-12-01",length(nat12)-1)))

nat13 <- read.csv(paste0(rawpath,"2013-Europa_SP2_BS_Nat_x_DG_201312.csv"), header=FALSE)
nat13 <- rbind(nat13, c("date", rep("2013-12-01",length(nat13)-1)))

nat14 <- read.csv(paste0(rawpath,"2014-Europa_SP2_BS_Nat_x_DG_201412.csv"), header=FALSE)
nat14 <- rbind(nat14, c("date", rep("2014-12-01",length(nat14)-1)))

nat15 <- read.csv(paste0(rawpath,"01012015 Statistical-Bulletin.csv"), header=FALSE)
nat15 <- rbind(nat15, c("date", rep("2015-01-01",length(nat15)-1)))

nat16 <- read.csv(paste0(rawpath,"Statistical-Bulleting 20160101.csv"), header=FALSE)
nat16 <- rbind(nat16, c("date", rep("2016-01-01",length(nat16)-1)))

nat17 <- read.csv(paste0(rawpath,"Statistical-Bulletin 20170101-18.csv"), header=FALSE)
nat17 <- rbind(nat17, c("date", rep("2017-01-01",length(nat17)-1)))

nat18_1 <- read.csv(paste0(rawpath,"01012018_NationalityAndDG.csv"), header=FALSE)
nat18_1 <- rbind(nat18_1, c("date", rep("2018-01-01",length(nat18_1)-1)))

nat18_10 <- read.csv(paste0(rawpath,"NationalityAndDG2018.csv"), header=FALSE)
nat18_10 <- rbind(nat18_10, c("date", rep("2018-10-01",length(nat18_10)-1)))

nat19_1 <- read.csv(paste0(rawpath,"20190101_NationalityAndDG.csv"), header=FALSE)
nat19_1 <- rbind(nat19_1, c("date", rep("2019-01-01",length(nat19_1)-1)))

nat19_4 <- read.csv(paste0(rawpath,"20190401_NationalityAndDG.csv"), header=FALSE)
nat19_4 <- rbind(nat19_4, c("date", rep("2019-04-03",length(nat19_1)-1)))

nat20 <- read.csv(paste0(rawpath,"01072020_NationalityAndDG.csv"), header=FALSE)
nat20 <- rbind(nat20, c("date", rep("2020-07-01",length(nat20)-1)))

nat21_1 <- read.csv(paste0(rawpath,"01012021_NationalityAndDG.csv"), header=FALSE)
nat21_1 <- rbind(nat21_1, c("date", rep("2021-01-01",length(nat21_1)-1)))

nat21_4 <- read.csv(paste0(rawpath,"01042021_NationalityAndDG.csv"), header=FALSE)
nat21_4 <- rbind(nat21_4, c("date", rep("2021-04-01",length(nat21_4)-1)))

nat21_7 <- read.csv(paste0(rawpath,"01072021_NationalityAndDG.csv"), header=FALSE)
nat21_7 <- rbind(nat21_7, c("date", rep("2021-07-01",length(nat21_7)-1)))

nat21_10 <- read.csv(paste0(rawpath,"01102021_NationalityAndDG.csv"), header=FALSE)
nat21_10 <- rbind(nat21_10, c("date", rep("2021-10-01",length(nat21_10)-1)))

nat22_1 <- read.csv(paste0(rawpath,"01012022_NationalityAndDG.csv"), header=FALSE)
nat22_1 <- rbind(nat22_1, c("date", rep("2022-01-01",length(nat22_1)-1)))

nat22_4 <- read.csv(paste0(rawpath,"01042022_NationalityAndDG.csv"), header=FALSE)
nat22_4 <- rbind(nat22_4, c("date", rep("2022-04-01",length(nat22_4)-1)))

nat22_7 <- read.csv(paste0(rawpath,"01072022_NationalityAndDG.csv"), header=FALSE)
nat22_7 <- rbind(nat22_7, c("date", rep("2022-07-01",length(nat22_7)-1)))

nat22_10 <- read.csv(paste0(rawpath,"01102022_NationalityAndDG.csv"), header=FALSE)
nat22_10 <- rbind(nat22_10, c("date", rep("2022-10-01",length(nat22_10)-1)))

nat23_1 <- read.csv(paste0(rawpath,"01012023_NationalityAndDG.csv"), header=FALSE)
nat23_1 <- rbind(nat23_1, c("date", rep("2023-01-01",length(nat23_1)-1)))

# data with 3 letter codes
c3 <- list(nat11, nat12, nat13, nat14, nat15, nat16, nat17, nat18_1, nat18_10, nat19_1, nat19_4, nat20)

# data with 2 letter codes
c2 <- list(nat21_1, nat21_4, nat21_7, nat21_10, nat22_1, nat22_4, nat22_7, nat22_10, nat23_1)
...
staff_c3_names <- merge(x = staff_c3, y = iso_alpha3, by.x = "c3", by.y = "code", all.x = TRUE)
...
staff_c2_names <- merge(x = staff_c2, y = iso_alpha2, by.x = "c2", by.y = "code", all.x = TRUE)
# test <- staff_c2_names[is.na(staff_c2_names$country),] #to check merge

c2_char <- subset(staff_c2_names, select = c("c2", "date", "country"))
c2_num <- select(staff_c2_names, -one_of(c("c2", "date", "country", "year", "cctld", "notes")))
...
staff_nat_avg <- staff %>% group_by(country, spring, year) %>%
  dplyr::summarise(ecfin = mean(ECFIN, na.rm = TRUE), total = mean(total, na.rm=TRUE), date = first(date))

tots <- staff_nat_avg %>% group_by(spring, year) %>%
  dplyr::summarise(total_ecfin = sum(ecfin, na.rm=TRUE), total_commission = sum(total, na.rm=TRUE))

staff_nat_avg$pct_ecfin <- staff_nat_avg$ecfin/staff_nat_avg$total

staff_nat_avg <- merge(staff_nat_avg, tots, by = c("spring", "year"))
staff_nat_avg$rate_ecfin <- (staff_nat_avg$ecfin/staff_nat_avg$total_ecfin)*100
staff_nat_avg$rate_commission <- (staff_nat_avg$total/staff_nat_avg$total_commission)*100

staff_nat_avg$country[staff_nat_avg$country == "United Kingdom of Great Britain and Northern Ireland"] <- "United Kingdom"
staff_nat_avg$country[staff_nat_avg$country == "Netherlands, Kingdom of the"] <- "Netherlands"
...
staff_nat <- staff_nat %>% mutate(ecfin_int = na.approx(ecfin), ecfin_spline = na.spline(ecfin))
