...
# Countries with EPU data in the analysis
epu_countries <- dfpg %>%
  filter(!is.na(epu), rev == 1 | exp == 1) %>%
  distinct(country) %>%
  pull(country) %>%
  sort()
cat(
  "Countries included in EPU analysis:\n",
  paste(epu_countries, collapse = ", "),
  "\n"
)
...
get_model_stats <- function(model) {
  ...
  ecfin <- get_coef_str(s, "ecfin")
  epu <- get_coef_str(s, "epu")
  r2_stats <- r2(model)
  list(
    coef_str = ecfin$coef_str,
    se_str = ecfin$se_str,
    epu_coef_str = epu$coef_str,
    epu_se_str = epu$se_str,
