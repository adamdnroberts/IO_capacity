...
save(coef_df, file = "~/ec_project/data/main_results_coef_df.Rdata")
...
delta_sens <- o_delta_rsq(
  y = "ln_err_sq",
  x = "ecfin",
  con = "ln_pop_int + ln_gdp + gdppc + country + ysp + title + py",
  type = "lm",
  data = dfpg_sens
)
print(delta_sens, n = 40)

sens_plot <- o_delta_rsq_viz(
  y = "ln_err_sq",
  x = "ecfin",
  con = "ln_pop_int + ln_gdp + gdppc + country + ysp + title + py",
  type = "lm",
...
png(
  "C:/Users/adamd/Dropbox/Apps/Overleaf/SYP Presentation/images/gdp_quartile_plot.png",
  width = 720,
  height = 480
)
par(cex.axis = 1.5, cex.lab = 1.5)
iplot(
  list(all_gdp, rev_gdp, exp_gdp),
  main = "",
  xlab = "GDP Quartile",
...
grayscale_colors <- gray.colors(3, start = 0.2, end = 0.8)
png(
  "C:/Users/adamd/Dropbox/Apps/Overleaf/SYP Presentation/images/gdppc_quartile_plot.png",
  width = 720,
  height = 480
)
par(cex.axis = 1.5, cex.lab = 1.5)
iplot(
  list(all_gdppc, rev_gdppc, exp_gdppc),
  main = "",
...
rev <- feols(
  log(err_sq) ~
    ecfin_int + log(pop_int) + log(gdp) + gdppc | country + ysp + title + py,
  data = subset(dfpg_2015, rev == 1)
)
exp <- feols(
  log(err_sq) ~
    ecfin_int + log(pop_int) + log(gdp) + gdppc | country + ysp + title + py,
  data = subset(dfpg_2015, exp == 1)
)
...
rev2 <- feols(
  log(err_sq) ~
    ecfin_int + log(pop_int) + log(gdp) + gdppc | country + ysp + title + py,
  data = subset(dfpg_noA2015, rev == 1)
)
exp2 <- feols(
  log(err_sq) ~
    ecfin_int + log(pop_int) + log(gdp) + gdppc | country + ysp + title + py,
  data = subset(dfpg_noA2015, exp == 1)
)
...
rev3 <- feols(
  log(err_sq) ~
    ecfin_int + log(pop_int) + log(gdp) + gdppc | country + ysp + title + py,
  data = subset(dfpg_noEOY2015, rev == 1)
)
exp3 <- feols(
  log(err_sq) ~
    ecfin_int + log(pop_int) + log(gdp) + gdppc | country + ysp + title + py,
  data = subset(dfpg_noEOY2015, exp == 1)
)
...
rev <- feols(
  log(err_sq) ~
    ecfin + log(pop_int) + log(gdp) + gdppc | country + ysp + title + py,
  data = subset(dfpg_covid, rev == 1)
)
exp <- feols(
  log(err_sq) ~
    ecfin + log(pop_int) + log(gdp) + gdppc | country + ysp + title + py,
  data = subset(dfpg_covid, exp == 1)
)
...
rev2 <- feols(
  log(err_sq) ~
    ecfin + log(pop_int) + log(gdp) + gdppc | country + ysp + title + py,
  data = subset(dfpg_noAcovid, rev == 1)
)
exp2 <- feols(
  log(err_sq) ~
    ecfin + log(pop_int) + log(gdp) + gdppc | country + ysp + title + py,
  data = subset(dfpg_noAcovid, exp == 1)
)
...
all3 <- feols(
  log(err_sq) ~
    ecfin + log(pop_int) + log(gdp) + gdppc | country + ysp + title + py,
  data = dfpg_noEOYcovid
)
rev3 <- feols(
  log(err_sq) ~
    ecfin + log(pop_int) + log(gdp) + gdppc | country + ysp + title + py,
  data = subset(dfpg_noEOYcovid, rev == 1)
)
exp3 <- feols(
  log(err_sq) ~
    ecfin + log(pop_int) + log(gdp) + gdppc | country + ysp + title + py,
  data = subset(dfpg_noEOYcovid, exp == 1)
)
...
ggsave(
  filename = "C:/Users/adamd/Dropbox/Apps/Overleaf/SYP Presentation/images/Exclude_Belgium_UK.png",
  plot = gdp_results,
  width = 6,
  height = 4
)
...
dfpg_robust_noA <- subset(
  dfpg_noA,
  country != "United Kingdom" & country != "Belgium"
)
dfpg_robust_noEOY <- subset(
  dfpg_noEOY,
  country != "United Kingdom" & country != "Belgium"
)
